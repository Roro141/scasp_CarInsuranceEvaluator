%%FACTS
ownerAge(jessie, 35).
mileage(jessie, 50000).
accidentHistory(jessie, 3).
violationHistory(jessie, 2).
credit(jessie, 700).
...
%% Define a rule to determine the insurance type for Jessie
...
%% Query the insurance type for Jessie
?- insuranceType(jessie, InsuranceType).
