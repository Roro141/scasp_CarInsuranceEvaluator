...
expensiveInsurance(X) :- dui(X, D), D >= 1.
expensiveInsurance(X) :- highRiskDriver(X), highMileage(X), not(standardAgeDriver(X)),badCredit(X).
expensiveInsurance(X) :- highRiskDriver(X), highMileage(X),standardAgeDriver(X), badCredit(X).
...
expensiveInsurance(X) :- safeDriver(X), highMileage(X),standardAgeDriver(X), badCredit(X).
...
%%?- insuranceType(jasmine, InsuranceType).
